...
export function HistoryPage() {
 ...
 return (
  <div className="min-h-screen bg-[#E8F5EC] pb-24 md:pb-8 font-sans">
    {/* Header */}
    <div className="bg-gradient-to-br from-[#0F6B3A] to-[#1E8449] text-white px-4 sm:px-6 md:px-10 pt-7 sm:pt-8 md:pt-10 pb-14 sm:pb-16 md:pb-20 relative overflow-hidden rounded-b-[2.5rem] shadow-xl z-10">
      {/* Soft overlay */}
      ...
      <div className="absolute bottom-0 left-0 w-72 h-72 md:w-96 md:h-96 bg-white/10 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2" />

      {/* Content container */}
      <div className="relative z-10 max-w-md sm:max-w-xl md:max-w-2xl lg:max-w-6xl mx-auto">
        {/* Top Row */}
        <div className="flex items-center justify-between mb-6">
          <button
            onClick={() => navigate("/patient/dashboard")}
            className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-white/10 backdrop-blur-md rounded-full hover:bg-white/20 transition-all text-sm font-medium active:scale-95 border border-white/10"
          >
        ...
        >
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-3">
            <div>
              ...
              <p className="text-emerald-100 text-sm sm:text-base opacity-90 font-light leading-relaxed">
                ติดตามประวัติผลเลือดและนัดหมายของคุณ
              </p>
            </div>

            {/* Desktop-only month hint */}
            <div className="hidden lg:flex items-center gap-2 text-emerald-100 text-sm">
              <span className="opacity-90">กำลังดู:</span>
 ...
 {/*Calendar Zone*/}
<div className="max-w-md md:max-w-2xl lg:max-w-6xl mx-auto px-4 -mt-10 relative z-20">
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div className="mt-12 lg:col-span-2">
      <motion.div
        ...
        className="w-full bg-white rounded-3xl shadow-xl border border-white/50 overflow-hidden backdrop-blur-sm"
      >
        {/* Calendar Header */}
        <div className="p-4 border-b border-slate-50 flex items-center justify-between bg-white/80 backdrop-blur-sm">
          ...
          <div className="flex items-center gap-1">
            <button
              onClick={prevMonth}
              className="p-2 hover:bg-[#E8F5EC] rounded-full transition-colors text-[#0F6B3A]"
            >
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    ${
                      isSelected
                        ? "bg-white ring-2 ring-[#0F6B3A] shadow-lg scale-110 z-10 font-bold text-[#0F6B3A]"
                        : "hover:bg-[#E8F5EC]"
                    }
                    ${
                      isTodayDate && !isSelected
                        ? "bg-[#E8F5EC] text-[#0F6B3A] font-semibold"
                        : ""
                    }
                  `}
                >
                  ...
                  <div className="absolute bottom-1 flex gap-0.5 justify-center">
                    {data?.inr && (
                      <div
                        ...
                        style={{
                          backgroundColor: getStatusInfo(data.inr.status).color,
                        }}
                      />
                    )}
                    {data?.appointment && (
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          {!selectedData && (
            ...
            >
              ...
              <p className="text-slate-500 font-medium">
                ไม่มีบันทึกข้อมูลสำหรับวันนี้
              </p>
              <p className="text-xs text-slate-400 mt-1">
                ข้อมูลจะปรากฏเมื่อโรงพยาบาลบันทึกเข้าระบบ
              </p>
            </motion.div>
          )}
          ...
          {selectedData?.medication && (
            <motion.div
              ...
              className={`rounded-3xl p-5 border relative overflow-hidden shadow-sm transition-all bg-white/90 backdrop-blur-md
                ${
                  selectedData.medication.status === "missed"
                    ? "border-red-100 ring-1 ring-red-100"
                    : selectedData.medication.status === "taken"
                    ? "border-[#0F6B3A]/20 ring-1 ring-[#0F6B3A]/10"
                    : "border-slate-100"
                }
              `}
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">
                      Warfarin {selectedData.medication.dose}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-4 flex items-center gap-2 text-sm text-slate-600 pl-1 border-t border-slate-100 pt-3">
                  ...
                  <span>
                    ...
                    <span className="font-medium text-slate-700">
                      {selectedData.medication.time} น.
                    </span>
                  </span>
                </div>
              )}
          ...
          {selectedData?.inr && (
            ...
            >
              <div
                className={`absolute top-0 right-0 w-32 h-32 rounded-bl-full opacity-5 transition-colors blur-xl pointer-events-none
                  ${
                    selectedData.inr.status === "safe"
                      ? "bg-[#0F6B3A]"
                      : selectedData.inr.status === "monitor"
                      ? "bg-amber-500"
                      : "bg-red-500"
                  }
                `}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      อัปเดตจากห้องปฏิบัติการ
                    </p>
                  </div>
                </div>

                <span
                  className={`px-3 py-1 rounded-full text-xs font-bold shadow-sm ${getStatusInfo(selectedData.inr.status).bgColor} ${getStatusInfo(selectedData.inr.status).textColor}`}
                >
              ...
              <div className="flex items-end gap-3 mb-4 pl-1">
                ...
                <div className="pb-2 text-sm font-medium text-slate-400">
                  ค่า INR
                </div>
              </div>
              ...
              {selectedData.inr.note && (
                <div className="bg-slate-50 rounded-xl p-3 text-sm text-slate-600 border border-slate-100">
                  <span className="font-bold text-slate-700 block mb-1 text-xs uppercase tracking-wider">
                    ผลวินิจฉัย
                  </span>
                  {selectedData.inr.note}
                </div>
              ...
              <div className="bg-white/60 rounded-2xl p-4 space-y-3 border border-white/60 shadow-sm">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-blue-900 font-medium">
                      {selectedData.appointment.time} น.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-blue-900 font-medium">
                      {selectedData.appointment.location}
                    </p>
                  </div>
                </div>
              </div>
